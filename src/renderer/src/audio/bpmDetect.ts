...
export function intervalToBpm(intervalSamples: number, sampleRate: number): number | null {
  ...
  let iterations = 0;
  while (bpm < BPM_MIN && iterations < 10) {
    bpm *= 2;
    iterations++;
  }
  iterations = 0;
  while (bpm > BPM_MAX && iterations < 10) {
    bpm /= 2;
    iterations++;
  }
...
export function detectBpmFromSamples(data: Float32Array, sampleRate: number): BpmResult {
  ...
  const confidence = second ? Math.min(1, top.count / (top.count + second.count)) : 1;
