...
function simpleSession(): SessionEvent[] {
  return [
    ...
    { elapsed_ms: 2000, type: 'set_volume', deck: 'A', gain: 0.5 },
...
describe('lane edit wrappers', () => {
  ...
  it('splices a gain gesture and restores the prior value at t1', () => {
    const events: SessionEvent[] = [{ elapsed_ms: 1000, type: 'set_volume', deck: 'A', gain: 0.8 }];
    const out = spliceLaneEvents(events, 'gain', 'A', 5000, 8000, [
      { ms: 5000, value: 0.4 },
      { ms: 6000, value: 0.4 }
    ]);
    const gains = out.filter((event) => event.type === 'set_volume').map((event) => event.gain);
...
describe('shared constants', () => {
  // mixer.ts cannot read WASM at module-evaluation time (init has not run yet),
  // so its copies are pinned to the Rust definitions here instead.
  it('mixer.ts constants match the session-core definitions', () => {
    const constants = editConstants();
    expect(EQ_MIN_DB).toBe(constants.eqMinDb);
    expect(EQ_MAX_DB).toBe(constants.eqMaxDb);
    expect(FILTER_DEAD_ZONE).toBe(constants.filterDeadZone);
    expect(DEFAULT_MASTER_GAIN).toBeCloseTo(constants.defaultMasterGain, 6);
