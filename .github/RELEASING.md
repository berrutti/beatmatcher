...
1. `auto-release` fires, bumps `package.json`, `src-tauri/tauri.conf.json`, and `src-tauri/Cargo.toml`, then pushes a `chore: release vX.Y.Z` commit plus a `vX.Y.Z` tag to main.
2. `build-and-release` fires from the main push. It builds the macOS `.dmg` and Windows `.exe` via Tauri and publishes them as a GitHub Release.
3. `deploy` runs in the same workflow. It only triggers when the commit message starts with `chore: release`, so it deploys to GitHub Pages exactly once per release and never on regular merges.
